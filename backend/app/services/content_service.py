...
# Import agents with new pipeline
try:
    from agents import ContentPipeline, InputType, PedagogyWorkflow
...
class ContentService:
    ...
    def __init__(self):
        ...
        if AGENTS_AVAILABLE:
            # Use new ContentPipeline for the 3-agent architecture
            self.pipeline = ContentPipeline()
            # Keep legacy workflow for backward compatibility
            self.workflow = self.pipeline  # PedagogyWorkflow is now alias to ContentPipeline
            logger.info("Content service initialized with ContentPipeline (3-agent architecture)")
        else:
            self.pipeline = None
            self.workflow = None
    ...
    def _build_educational_script_from_fda(
        self,
        pipeline_result: Dict[str, Any],
        topic: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        Build an EducationalScript-compatible dict from FDA/pipeline output.
        
        This ensures backward compatibility with the ContentResponse model
        which expects educational_script to have a 'title' field.
        
        Args:
            pipeline_result: Result from the content pipeline
            topic: Optional topic override
            
        Returns:
            Dictionary compatible with EducationalScript model
        """
        fda = pipeline_result.get("fda", {})
        metadata = pipeline_result.get("metadata", {})
        
        # Extract title from FDA or metadata or use default
        title = (
            fda.get("title")
            or topic
        ...
        # Convert FDA slides to sections
        sections = []
        for slide in fda.get("slides", []):
            section = {
                "title": slide.get("title", f"Section {slide.get('slide_number', 1)}"),
                "content": slide.get("content", {}).get("text", ""),
                "learning_objectives": [],
                "animations": [
                    {
                        "name": rule.get("rule_id", "animation"),
                        "description": rule.get("intent", "animation"),
                        "concepts": [],
                        "duration_seconds": rule.get("timing", {}).get("duration_seconds", 2),
                        "complexity": "medium"
                    }
                    for rule in slide.get("animation_rules", [])
                ],
                "duration_minutes": int(slide.get("duration_seconds", 60) / 60) or 1,
                "prerequisites": []
            }
        ...
        return {
            "title": title,
            "description": fda.get("topic", topic or ""),
            "learning_objectives": [],
            "sections": sections,
            "assessments": [],
            "metadata": {
                "difficulty_level": fda.get("difficulty_level", "intermediate"),
                "target_audience": fda.get("target_audience", "general"),
                "estimated_duration": f"{int(fda.get('total_duration_seconds', 60) / 60)} minutes",
                "total_slides": len(fda.get("slides", [])),
                "pipeline_version": metadata.get("pipeline_version", "2.0"),
            },
            "estimated_duration_minutes": int(fda.get("total_duration_seconds", 60) / 60) or 1,
            "prerequisites": [],
            "tags": []
        }
        ...
        try:
            ...
            for i, input_item in enumerate(inputs):
                ...
                if content_type == "text" and content.strip():
                    ...
                    validation_result = (
                        await self.content_validator.validate_text_content(
                            content,
                            f"multi_input_{i}",
                            {
                                **metadata,
                    ...
                    if not validation_result.is_valid:
                        ...
                        logger.error(
                            f"Multi-input validation failed for {generation_id}: {error_message}"
                        )
                        error_result = {
                            "generation_id": generation_id,
                            "status": "failed",
            ...
            if not full_content.strip():
                raise ValueError(
                    "No valid text content found in inputs after validation"
                )
            ...
            if all_warnings:
                logger.warning(
                    f"Content validation warnings for {generation_id}: {all_warnings}"
                )
            ...
            content_input = ContentInput(
                ...
                metadata={
                    ...
                    "original_inputs": [
                        {
                            "content_type": (
                                inp.get("content_type")
                                if isinstance(inp, dict)
                                else inp.content_type
                            ),
                            "content_length": len(
                                inp.get("content", "")
                                if isinstance(inp, dict)
            ...
            try:
                ...
                logger.warning(
                    f"Failed to store multi-input for {generation_id}: {str(storage_error)}"
                )
                storage_id = None

            if AGENTS_AVAILABLE and self.pipeline:
                # Use new ContentPipeline
                context = {
                    "topic": topic,
                    "difficulty_level": difficulty_level,
                    "target_audience": target_audience,
                ...
                # Combined text content uses TEXT input type
                ...
                result = {
                    "generation_id": generation_id,
                    "status": "completed",
                    "educational_script": self._build_educational_script_from_fda(pipeline_result, topic),
                    "video_url": pipeline_result.get("video_url"),
                    "video_path": pipeline_result.get("video_path"),
                    "fda": pipeline_result.get("fda"),
                    "manim_code": pipeline_result.get("manim_code"),
                    "created_at": datetime.now(),
                ...
                result = self._create_mock_response(
                    generation_id, topic or "Generated Content"
                )
    ...
    async def generate_from_text(
        ...
        try:
            ...
            if not validation_result.is_valid:
                ...
                logger.error(
                    f"Text validation failed for {generation_id}: {error_message}"
                )
                error_result = {
                    "generation_id": generation_id,
                    "status": "failed",
            ...
            if validation_result.warnings:
                logger.warning(
                    f"Content validation warnings for {generation_id}: {validation_result.warnings}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to store input for {generation_id}: {str(storage_error)}"
                )
                storage_id = None

            if AGENTS_AVAILABLE and self.pipeline:
                # Use new ContentPipeline with TEXT input type
                # This will: Pedagogical Agent -> FDA Agent -> Manim Agent
                context = {
                    "topic": topic,
                    "difficulty_level": difficulty_level,
                    "target_audience": target_audience,
                ...
                # Execute pipeline with TEXT input type (uses Pedagogical Agent)
                pipeline_result = self.pipeline.execute(
                    processed_content, 
                    InputType.TEXT, 
                    context
            ...
            if storage_id:
                try:
                    ...
                    logger.warning(
                        f"Failed to update processing metadata: {str(update_error)}"
                    )
        ...
        try:
            ...
            try:
                ...
                if not file_validation.is_valid:
                    ...
                    logger.error(
                        f"File validation failed for {generation_id}: {error_message}"
                    )
                    error_result = {
                        "generation_id": generation_id,
                        "status": "failed",
                ...
                if file_validation.warnings:
                    logger.warning(
                        f"File validation warnings for {generation_id}: {file_validation.warnings}"
                    )
            ...
            try:
                extraction_result = await self.document_parser.extract_text_from_file(
                    file
                )
                ...
                logger.info(
                    f"Successfully extracted {len(text_content)} characters from {file.filename}"
                )
            ...
            if not content_validation.is_valid:
                ...
                logger.error(
                    f"Content validation failed for {generation_id}: {error_message}"
                )
                error_result = {
                    "generation_id": generation_id,
                    "status": "failed",
            ...
            if all_warnings:
                logger.warning(
                    f"File content validation warnings for {generation_id}: {all_warnings}"
                )
            ...
            try:
                storage_id = await input_storage_service.store_input(
                    ...
                    validation_result={
                        "file_validation": (
                            file_validation.__dict__
                            if hasattr(file_validation, "__dict__")
                            else file_validation
                        ),
                        "content_validation": (
                            content_validation.__dict__
                            if hasattr(content_validation, "__dict__")
                ...
                logger.warning(
                    f"Failed to store file input for {generation_id}: {str(storage_error)}"
                )
                storage_id = None

            # Generate content using FILE input type (skips Pedagogical Agent)
            if AGENTS_AVAILABLE and self.pipeline:
                context = {
                ...
                # FILE input type skips Pedagogical Agent: FDA -> Manim
                pipeline_result = self.pipeline.execute(
                    processed_content,
                    InputType.FILE,
                    context
                ...
                result = {
                    "generation_id": generation_id,
                    "status": "completed",
                    "educational_script": self._build_educational_script_from_fda(pipeline_result, topic or file.filename),
                    "video_url": pipeline_result.get("video_url"),
                    "video_path": pipeline_result.get("video_path"),
                    "fda": pipeline_result.get("fda"),
                    "manim_code": pipeline_result.get("manim_code"),
                ...
                # Mock response for testing
                result = self._create_mock_response(
                    generation_id, topic or file.filename
                )
                result["validation_warnings"] = all_warnings
    ...
    async def generate_from_url(
        ...
        try:
            ...
            if not url_validation.is_valid:
                error_message = (
                    f"URL validation failed: {'; '.join(url_validation.errors)}"
                )
                logger.error(
                    f"URL validation failed for {generation_id}: {error_message}"
                )
                error_result = {
                    "generation_id": generation_id,
                    "status": "failed",
            ...
            if all_warnings:
                logger.warning(
                    f"URL content validation warnings for {generation_id}: {all_warnings}"
                )
            ...
            try:
                storage_id = await input_storage_service.store_input(
                    ...
                    validation_result={
                        "url_validation": (
                            url_validation.__dict__
                            if hasattr(url_validation, "__dict__")
                            else url_validation
                        ),
                        "content_validation": (
                            content_validation.__dict__
                ...
                logger.warning(
                    f"Failed to store URL input for {generation_id}: {str(storage_error)}"
                )
                storage_id = None

            # Generate content using URL input type (skips Pedagogical Agent)
            if AGENTS_AVAILABLE and self.pipeline:
                context = {
                ...
                # URL input type skips Pedagogical Agent: FDA -> Manim
                pipeline_result = self.pipeline.execute(
                    processed_content,
                    InputType.URL,
                    context
                ...
                result = {
                    "generation_id": generation_id,
                    "status": "completed",
                    "educational_script": self._build_educational_script_from_fda(pipeline_result, topic or self._extract_title_from_url(url)),
                    "video_url": pipeline_result.get("video_url"),
                    "video_path": pipeline_result.get("video_path"),
                    "fda": pipeline_result.get("fda"),
                    "manim_code": pipeline_result.get("manim_code"),
                ...
                # Mock response for testing
                result = self._create_mock_response(
                    generation_id, topic or self._extract_title_from_url(url)
                )
                result["validation_warnings"] = all_warnings
                result["content_metadata"] = {
                    **content_validation.metadata,
    ...
    async def _scrape_url_content(self, url: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"Successfully extracted {len(cleaned_content)} characters from URL: {url}"
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to scrape URL content: {str(e)}"
            )
    ...
    async def _make_request_with_retry(
        self, url: str, headers: Dict[str, str], max_retries: int = 2
    ) -> requests.Response:
    ...
    def _parse_html_content(
        self, content: bytes, encoding: Optional[str] = None
    ) -> BeautifulSoup:
    ...
    def _handle_http_error(self, error: requests.exceptions.HTTPError) -> HTTPException:
        ...
        detail = error_messages.get(
            status_code, f"HTTP error {status_code}: {error.response.reason}"
        )
    ...
    def _validate_content_quality(self, content: str, url: str) -> None:
        ...
        for indicator in low_quality_indicators:
            if indicator in content_lower:
                logger.warning(
                    f"Potential content quality issue detected for {url}: {indicator}"
                )
                break
        ...
        if len(words) > 50:
            ...
            if diversity_ratio < 0.3:  # Less than 30% unique words
                logger.warning(
                    f"Low content diversity detected for {url}: {diversity_ratio:.2f}"
                )

        logger.info(
            f"Content quality validation passed for {url}: {content_length} characters"
        )
    ...
    def _remove_unwanted_elements(self, soup: BeautifulSoup) -> None:
        ...
        for pattern in unwanted_patterns:
            ...
            for element in soup.find_all(
                class_=lambda x: x and any(pattern in cls.lower() for cls in x)
            ):
                element.decompose()
            # Remove by id
    ...
    def _extract_text_from_element(self, element) -> str:
        ...
        text_elements = element.find_all(
        ...
        for elem in text_elements:
            ...
            if (
                len(text) < 5  # Reduced from 10 to 5 to capture more content
                or text in seen_texts
                or not self._is_meaningful_text(text)
            ):
                continue
        ...
        try:
            ...
            if path_parts:
                ...
                if title_part and len(title_part) > 2:
                    # Capitalize properly
                    title_part = " ".join(
                        word.capitalize() for word in title_part.split()
                    )
            ...
            return [
                {
                    ...
                    "topic": (
                        gen_data.get("educational_script", {}).get("title")
                        if gen_data.get("educational_script")
                        else None
                    ),
                }
    ...
    def _create_mock_response(self, generation_id: str, title: str) -> Dict[str, Any]:
        ...
        educational_script = {
            ...
            "learning_objectives": [
                ...
                {
                    "objective": "Apply knowledge to examples",
                    "level": "apply",
                    "measurable": True,
                },
            ],
    ...
    async def generate_manim_code(self, generation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not educational_script:
                raise ValueError(
                    f"No educational script found for generation {generation_id}"
                )
    ...
    async def render_video(
        ...
        try:
            ...
            # Check if Manim code exists - handle both string and dict formats
            manim_code_field = generation.get("manim_code")
            if not manim_code_field:
                raise ValueError(
                    f"No Manim code found for generation {generation_id}. Generate Manim code first."
                )

            # Handle both formats:
            # - New pipeline: manim_code is directly a string
            # - Legacy: manim_code is a dict with "manim_code" key
            if isinstance(manim_code_field, str):
                manim_code = manim_code_field
            elif isinstance(manim_code_field, dict):
                manim_code = manim_code_field.get("manim_code", "")
            else:
                raise ValueError(f"Invalid manim_code format for generation {generation_id}")
            
            if not manim_code:
                raise ValueError(f"Manim code is empty for generation {generation_id}")

            logger.info(f"Rendering video for generation {generation_id}")

            # Render video and upload to S3
            render_result = await self.video_renderer.render_and_upload(
                manim_code=manim_code,
                generation_id=generation_id,
                quality=quality,
            ...
            self.generations[generation_id]["video"] = render_result

            if render_result.get("success"):
                logger.info(
                    f"Successfully rendered video for {generation_id}: {render_result.get('s3_url')}"
                )
            else:
                logger.error(
                    f"Video rendering failed for {generation_id}: {render_result.get('error')}"
                )
