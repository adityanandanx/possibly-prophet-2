...
async def _validate_doc_file(content: bytes, file_ext: str) -> None:
    ...
    if file_ext == '.docx':
        ...
        if not (content.startswith(b'\xd0\xcf\x11\xe0') or content.startswith(b'\x0d\x44\x4f\x43')):
            raise HTTPException(
                status_code=415,
...
async def _validate_text_file(content: bytes) -> None:
    ...
    try:
        ...
        try:
            ...
            if control_chars > len(text_content) * 0.1:  # More than 10% control characters
                raise HTTPException(
                    status_code=415,
...
async def generate_content(request: ContentRequest) -> ContentResponse:
    ...
    try:
        ...
        if not request.inputs:
            raise HTTPException(
                status_code=400,
                detail="At least one content input is required."
            )
...
async def generate_from_text_prompt(
    ...
    difficulty_level: str = Form("intermediate", description="Content difficulty level"),
    ...
    try:
        ...
        if learning_goals.strip():
            parsed_learning_goals = [goal.strip() for goal in learning_goals.split(",") if goal.strip()]
        ...
        raise HTTPException(status_code=500, detail=f"Text prompt processing failed: {str(e)}")

@router.post("/upload", response_model=ContentResponse)
async def upload_and_generate(
    file: UploadFile = File(..., description="Educational content file (PDF, DOC, DOCX, TXT)"),
    topic: Optional[str] = Form(None, description="Optional topic override"),
    difficulty_level: str = Form("intermediate", description="Content difficulty level"),
    target_audience: str = Form(..., description="Target audience for content"),
    learning_goals: str = Form("", description="Comma-separated learning goals")
    ...
    """
    try:
        logger.info(f"Processing uploaded file: {file.filename} (size: {file.size if hasattr(file, 'size') else 'unknown'})")
        ...
        if not target_audience.strip():
            raise HTTPException(
                status_code=422,
                detail="Target audience cannot be empty"
            )
        ...
        logger.info(f"Successfully processed file: {file.filename}, generation_id: {result.get('generation_id')}")
        return ContentResponse(**result)
    ...
    try:
        ...
        try:
            ...
            if not parsed.netloc:
                raise HTTPException(
                    status_code=400,
                    detail="Invalid URL format. Missing domain name."
                )
        except Exception:
            raise HTTPException(
                status_code=400,
        ...
        if learning_goals.strip():
            ...
            if len(parsed_learning_goals) > 10:
                raise HTTPException(
                    status_code=422,
                    detail="Too many learning goals. Maximum is 10."
                )
        ...
        logger.info(f"Successfully processed URL: {url}, generation_id: {result.get('generation_id')}")
        return ContentResponse(**result)
...
async def validate_text_content(
    ...
    try:
        ...
        response = {
            ...
            "sanitized_content_length": len(validation_result.sanitized_content) if validation_result.sanitized_content else 0,
        ...
        if validation_result.metadata.get("quality_score", 0) < 0.5:
            response["recommendations"].append("Consider adding more detailed explanations and examples")
        
        if validation_result.metadata.get("educational_score", 0) < 0.3:
            response["recommendations"].append("Add more educational context and learning-focused language")
        
        if validation_result.metadata.get("word_count", 0) < 100:
            response["recommendations"].append("Content is quite short - consider expanding with more details")
        
        if not validation_result.metadata.get("has_structure", False):
            response["recommendations"].append("Consider adding headings or bullet points to improve structure")
        
        logger.info(f"Text validation completed: valid={validation_result.is_valid}, quality={validation_result.metadata.get('quality_score', 0):.2f}")
        return response
...
async def validate_url_content(
    ...
    try:
        ...
        if validation_result.metadata.get("domain"):
            ...
            if any(indicator in domain.lower() for indicator in ["blog", "wiki", "edu", "org"]):
                response["recommendations"].append("This appears to be an educational or informational site - good for content generation")
            elif any(indicator in domain.lower() for indicator in ["shop", "store", "buy", "sell"]):
                response["recommendations"].append("This appears to be a commercial site - may have limited educational content")
        
        if validation_result.warnings:
            response["recommendations"].append("Review warnings before proceeding with content extraction")
        
        logger.info(f"URL validation completed: valid={validation_result.is_valid}, domain={validation_result.metadata.get('domain', 'unknown')}")
        return response
...
async def validate_batch_content(request: ContentRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        validation_results = await content_validator.validate_batch_content(content_items)
        
        # Prepare response
        response = {
            "total_inputs": len(request.inputs),
            "valid_inputs": sum(1 for result in validation_results if result.is_valid),
            "invalid_inputs": sum(1 for result in validation_results if not result.is_valid),
            "results": [],
        ...
        if response["invalid_inputs"] > 0:
            response["overall_recommendations"].append(f"{response['invalid_inputs']} inputs failed validation - review errors before proceeding")
        
        total_warnings = sum(len(result.warnings) for result in validation_results)
        if total_warnings > 0:
            response["overall_recommendations"].append(f"{total_warnings} warnings found across all inputs - review for quality improvements")
        
        if response["valid_inputs"] > 0:
            avg_quality = sum(result.metadata.get("quality_score", 0) for result in validation_results if result.is_valid) / response["valid_inputs"]
            if avg_quality < 0.5:
                response["overall_recommendations"].append("Average content quality is low - consider improving content before generation")
        
        logger.info(f"Batch validation completed: {response['valid_inputs']}/{response['total_inputs']} valid inputs")
        return response
